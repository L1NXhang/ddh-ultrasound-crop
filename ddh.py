"""DDH crop research core v2. No patient identities, no diagnostic claims.
...
def loss_crop(out,batch,baseline=False):
...
def training(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in dl:
            ...
            if is_b:
                b=moved(batch,dev);out=model(b['image'],b['roi']);loss=loss_crop(out,b,args.baseline)
            else:
        ...
        with torch.no_grad():
            for batch in vl:
                if is_b:
                    b=moved(batch,dev);out=model(b['image'],b['roi']);values.append((float(loss_crop(out,b,args.baseline)),len(b['image'])))
                else:
                    images,targets=batch;outs=model([x.to(dev) for x in images])
                    for o,t in zip(outs,targets):
                        # Missing detections score zero IoU, never disappear from denominator.
                        iou=float(box_iou(o['boxes'][:1].cpu(),t['boxes'])[0,0]) if len(o['boxes']) else 0
